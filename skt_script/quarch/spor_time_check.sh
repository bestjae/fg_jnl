...
echo "nvme format ... "
nvme format $DEV_NAME

echo "Fio ... "
fio --bs=4k --ioengine=libaio --iodepth=32 --direct=1 --numjobs=4 --thread --group_reporting --filename=${DEV_NAME} --name=rand-write --rw=randwrite

for((i=0;i<10;i++))
do
    echo "Fio ... "
    fio --bs=4k --ioengine=libaio --iodepth=32 --direct=1 --runtime=60 --numjobs=4 --thread --group_reporting --filename=${DEV_NAME} --name=rand-write --rw=randwrite
