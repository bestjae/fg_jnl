...
if [ $argc -lt 2 ]; then
    echo "Usage : ./ns_test_22.sh [device] [test_count]"
...
echo "<Test scenario : Multiple namespace format -> power cycle -> format verify start>"
...
do
    ...
    do
        echo "format namespace $j"
        ${script_dir}/../../../micro/format.sh $dev_name $j
        echo "format namespace $j, secure erase option 0"
        ${script_dir}/../../../micro/format.sh $dev_name $j 0
        echo "format namespace $j, secure erase option 1"
        ${script_dir}/../../../micro/format.sh $dev_name $j 1
	echo "format namespace $j, secure erase option 2"
        ${script_dir}/../../../micro/format.sh $dev_name $j 2
