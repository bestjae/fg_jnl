...
if [ $argc -lt 2 ]; then
    echo "Usage : ./ns_test_02.sh [device] [test_count]"
...
echo "<Test scenario : Namespace create -> detach -> attach test>"
for (( j=1;j<=$test_count;j++ ))
do 
    echo "Test scenario progress :" $j"/"$test_count 

    for (( k=1;k<=$total_support_ns;k++ )) do 
        echo "(create" $k "namesapce -> detach" $k "namesapce" " -> attach" $k "namesapce)"
        ...
        for (( l=1;l<=k;l++ ))
        do
            ${script_dir}/../../../micro/detach_ns1.sh $dev_name $l $controller_id
        done
        
        for (( l=1;l<=k;l++ ))
        do
            ${script_dir}/../../../micro/attach_ns1.sh $dev_name $l $controller_id
        done
        del_all
    done
    echo -e "\n"
