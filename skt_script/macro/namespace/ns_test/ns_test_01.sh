...
if [ $argc -lt 2 ]; then
    echo "Usage : ./ns_test_01.sh [device] [test_count]"
...
echo "<Test scenario : Namespace create -> delete test>"
for (( i=1;i<=$test_count;i++ ))
do
    echo "Test scenario progress :" $i"/"$test_count 
    for (( j=1;j<=$total_support_ns;j++ )) do 
        echo "(create" $j "namesapce -> delete" $j "namesapce)"
        for (( k=1;k<=j;k++ )) 
        do
