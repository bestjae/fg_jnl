...
if [ $argc -lt 2 ]; then
    echo "Usage : ./ns_test_23.sh [device] [test_count]"
...
echo "<Test scenario : Various size namespace create -> namespace nvmcap check>"
for (( j=1;j<=$test_count;j++ ))
do
	echo "Test scenario progress :" $j"/"$test_count 
	for (( k=1;k<=$total_support_ns;k++ ))
	do
	    ${script_dir}/../../../micro/create_ns1.sh $dev_name ${size[$(($k-1))]} > /dev/null 2> /dev/null  
	    ${script_dir}/../../../micro/attach_ns1.sh $dev_name $k $controller_id > /dev/null 2> /dev/null
	done
	${script_dir}/../../check_list/power_cycle.sh 
	
	for (( k=1;k<=$total_support_ns; k++ ))
	do
    		lbsize=`nvme id-ns $dev_name -n $k | grep nvmcap | awk '{print $3}'`
		echo "lbsize =" $lbsize
			#namespace size calculate
		    crtsize=`expr ${size[$((k-1))]} \* 512`
		
		    if [ $lbsize -eq $crtsize ]
		    then
    			echo "namespace $l nvmcap correct ($lbsize = $crtsize)"
		    else
    			echo "namespace $l nvmcap uncorrect ($lbsize = $crtsize)"
				echo "Test fail"
				exit 1
		    fi
