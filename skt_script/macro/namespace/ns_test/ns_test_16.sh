...
if [ $argc -lt 2 ]; then
    echo "Usage : ./ns_test_16.sh [device] [test_count]"
...
echo "<Test scenario : Namespace create-delete-detach test>"
for (( j=1;j<=$test_count;j++ ))
do
    echo "Test scenario progress :" $j"/"$test_count 
    for (( k=1;k<=$total_support_ns;k++ ))
    do
        echo "(create" $k "namespace -> delete" $k "namespace -> detach" $k "namespace)"
        for (( l=1;l<=k;l++ )) 
        do
        ...
        for (( l=1;l<=k;l++ )) 
        do
            ${script_dir}/../../../micro/delete_ns1.sh $dev_name $l
        done 
