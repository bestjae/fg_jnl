#!/bin/bash
script_dir=$(cd "$(dirname "$0")" && pwd) argc=$#
dev_name=$1
firmware_path=$2
test_count=$3
...
if [ $argc -lt 2 ]; then
    echo "Usage : ./action1.sh [dev_name] [firmware_path]"
...
do
    ...
    do
        ...
        do
            ...
            echo " (Download firmware testfw0"$slot")"
            if [ $slot -eq $target_slot ]; then
                ${script_dir}/../../micro/fw-activate.sh $dev_name 1 $slot
            else
