...
if [ $argc -lt 1 ]; then
    echo "Usage : ./smart_io_test.sh [device]"
...
echo "<Test scenario : smart-log value test with I/O>"
...
do
    ...
    do
        echo "Start I/O on namespace" $k
        for (( l=0;l<50;l++ ))
        do
            echo "write ${dev_name}n${k} ${l} ${request_size}"
            ${script_dir}/../../../micro/write.sh ${dev_name}n${k} ${l} ${request_size} 
            echo "read ${dev_name}n${k} ${l} ${request_size}"
	        ${script_dir}/../../../micro/read.sh ${dev_name}n${k} ${l} ${request_size} 
        done
        echo "End I/O on namespace" $k
    done
    echo -e "\n"
done
echo -e "\n"
