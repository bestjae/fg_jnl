...
if [ $argc -lt 2 ]; then
    echo "Usage : ./dsm_test_03.sh [device name] [test_count]"
...
echo "<Test scenario : namespace write -> dsm -> flush -> read -> I/O verify test>"
...
do
    ...
    do
		...
		echo "${dev_name}n${k} dsm verify with flush success"
    done
	echo -e "\n"
done
