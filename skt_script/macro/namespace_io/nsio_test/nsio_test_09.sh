...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_04.sh [device name] [test_count]"
...
echo "<Test scenario : (/dev/nvme0 : reset), (/dev/nvme0n1~nvme0n8 : read/write)>"
for (( i=1;i<=$test_count;i++ ))
do
    echo "Test scenario progress :" $i"/"$test_count 
#${script_dir}/../../../micro/reset.sh $dev_name
    printf "/dev/nvme0n1 -> "
    ${script_dir}/../../../micro/read.sh $dev_name"n"1  0 1024 100 
    printf "/dev/nvme0n2 -> "
    ${script_dir}/../../../micro/write.sh $dev_name"n"2 1024 100 
    printf "/dev/nvme0n3 -> "
    ${script_dir}/../../../micro/read.sh $dev_name"n"3 0 1024 100 
    printf "/dev/nvme0n4 -> "
