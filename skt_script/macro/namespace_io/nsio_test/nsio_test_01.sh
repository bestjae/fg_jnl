...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_01.sh [device name] [test_count]"
...
echo "<Test scenario : (/dev/nvme0n1~nvme0n4 : get-log), (/dev/nvme0n5~nvme0n8 : read/write)>"
for (( i=1;i<=$test_count;i++ ))
do
    echo "Test scenario progress :" $i"/"$test_count 
    echo "get_log -> /dev/nvme0n1,2,3,4"
    ...
    ${script_dir}/../../../micro/get_log.sh $dev_name 1 512 3 > /dev/null 2> /dev/null
    ${script_dir}/../../../micro/get_log.sh $dev_name 1 512 4 > /dev/null 2> /dev/null
    printf "/dev/nvme0n5 -> "
    ${script_dir}/../../../micro/read.sh $dev_name"n"5  1024 100 
    printf "/dev/nvme0n6 -> "
    ${script_dir}/../../../micro/write.sh $dev_name"n"6 1024 100 
    printf "/dev/nvme0n7 -> "
    ${script_dir}/../../../micro/read.sh $dev_name"n"7 1024 100 
    printf "/dev/nvme0n8 -> "
