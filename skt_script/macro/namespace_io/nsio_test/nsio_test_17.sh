...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_17.sh [device name] [test_count]"
...
echo "<Test scenario : namespace write-uncorrectable -> read test>"
...
do
    ...
    do
    	for (( l=1;l<2;l++ ))
    	do
	    	echo "write-uncorrectable ${dev_name}n${k}"
			${script_dir}/../../../micro/write-uncor.sh ${dev_name}n${k} ${start_block} ${request_size} 
