...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_07.sh [device name] [test_count]"
...
echo "<Test scenario : (/dev/nvme0n1~nvme0n2 : get_log), (/dev/nvme0n3~nvme0n4 : smart_log)"
echo "                 (/dev/nvme0n5~nvme0n6 : error_log), (/dev/nvme0n7~nvme0n8 : read/write)>"
for (( i=1;i<=$test_count;i++ ))
do
    echo "Test scenario progress :" $i"/"$test_count 
    echo "get_log -> /dev/nvme0n1,2"
    ${script_dir}/../../../micro/get_log.sh $dev_name 1 512 1 > /dev/null 2> /dev/null
    ${script_dir}/../../../micro/get_log.sh $dev_name 1 512 2 > /dev/null 2> /dev/null
    echo "smart_log -> /dev/nvme0n3,4"
    ${script_dir}/../../../micro/smart_log.sh $dev_name 3 > /dev/null 2> /dev/null
    ${script_dir}/../../../micro/smart_log.sh $dev_name 4 > /dev/null 2> /dev/null
    echo "error_log -> /dev/nvme0n5,6"
    ${script_dir}/../../../micro/error_log.sh $dev_name 64 5 > /dev/null 2> /dev/null
    ${script_dir}/../../../micro/error_log.sh $dev_name 64 6 > /dev/null 2> /dev/null
    printf "/dev/nvme0n7 -> "
