...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_19.sh [device name] [test_count]"
...
echo "<Test scenario : namespace write zero -> read test>"
...
do
    ...
    do
    	...
    	do
	    	...
	    	echo "write-zeroes ${dev_name}n${k} (start block : $l, request_size :${request_size} sector)"  
		...
		echo "${dev_name}n${k} zero I/O verify success"
	    echo -e "\n"
    done
	echo -e "\n"
done
