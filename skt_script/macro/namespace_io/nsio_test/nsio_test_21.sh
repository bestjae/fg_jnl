...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_21.sh [device name] [test_count]"
...
echo "<Test scenario : namespace write -> controller reset -> read test>"
...
#${script_dir}/../../../micro/reset.sh ${dev_name} > /dev/null 2> /dev/null
#echo "reset"
		...
		echo "${dev_name}n${k} I/O verify with reset success"
	    echo -e "\n"
    done
	echo -e "\n"
done
