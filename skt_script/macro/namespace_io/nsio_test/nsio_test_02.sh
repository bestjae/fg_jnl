...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_02.sh [device name] [test_count]"
...
echo "<Test scenario : (/dev/nvme0n1~nvme0n4 : smart-log), (/dev/nvme0n5~nvme0n8 : read/write)>"
for (( i=1;i<=$test_count;i++ ))
do
    echo "Test scenario progress :" $i"/"$test_count 
    echo "smart_log -> /dev/nvme0n1,2,3,4"
    ${script_dir}/../../../micro/smart_log.sh $dev_name 1 > /dev/null 2> /dev/null
    ...
    printf "/dev/nvme0n5 -> "
    ${script_dir}/../../../micro/read.sh $dev_name"n"5  0 1024 100 
    printf "/dev/nvme0n6 -> "
