...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_22.sh [device name] [test_count]"
...
echo "<Test scenario : namespace write -> flush -> power cycle -> read test>"
...
do
    ...
    do
    	...
    	do
            ...
            ${script_dir}/../../../micro/flush.sh $dev_name $k 
			
	    	echo "subsystem-reset"
#${script_dir} ./../../../micro/subsystem-reset.sh ${dev_name} > /dev/null 2> /dev/null
		...
		echo "${dev_name}n${k} I/O verify with subsystem success"
	    echo -e "\n"
    done
	echo -e "\n"
done
