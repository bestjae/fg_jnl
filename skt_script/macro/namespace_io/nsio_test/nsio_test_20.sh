...
if [ $argc -lt 2 ]; then
    echo "Usage : ./nsio_test_20.sh [device name] [test_count]"
...
echo "<Test scenario : namespace write zero -> power cycle -> read test>"
...
do
    ...
    do
    	...
    	do
			...
			${script_dir}/../../../micro/write-zeroes.sh ${dev_name}n${k} ${l} ${request_size} > /dev/null 2> /dev/null
	    	echo "write-zeores ${dev_name}n${k} (start block : $l, request_size :${request_size} sector)"  
		...
		echo "${dev_name}n${k} zero I/O verify with power cycle  success"
	    echo -e "\n"
    done
	echo -e "\n"
done
