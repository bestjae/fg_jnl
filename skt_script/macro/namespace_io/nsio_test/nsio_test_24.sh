...
if [ $argc -lt 2 ]; then
	echo "Usage : ./nsio_test_24.sh [device name] [test_count]"
...
do
	...
	do
		...
		do
			...
			${script_dir}/../../../micro/read.sh ${dev_name}n${l} ${start_block} ${request_size} 
			if [ $l -ne $k ]
			then
	...
	echo -e "\n"
	start_block=`expr $start_block + 100`
done
